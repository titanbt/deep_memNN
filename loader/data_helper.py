...
class DataHelper(object):
    def __init__(self, train_file, test_file, edim, init_std,
                 embedding_path=None):
    ...
    def load_data(self):
        ...
        self.train_data = self._read_data(self.train_file, source_count, source_word2idx, target_count, target_word2idx)
        self.test_data = self._read_data(self.test_file, source_count, source_word2idx, target_count, target_word2idx)

        self.params['pad_idx'] = source_word2idx['<pad>']
        self.params['nwords'] = len(source_word2idx)
        self.params['mem_size'] = self.train_data[4] if self.train_data[4] > self.test_data[4] else self.test_data[4]

        print "Loading word2vec..."
        self.params['embedd_table'] = self.init_word_embeddings(source_word2idx)
        self.params['embedd_table_aspect'] = self.init_word_embeddings(target_word2idx)

        return self.train_data, self.test_data, self.params

    def _read_data(self, fname, source_count, source_word2idx, target_count, target_word2idx):
            ...
            if len(text.strip()) != 0:
                ...
                for asp_terms in sentence.iter('aspectTerms'):
                    for asp_term in asp_terms.findall('aspectTerm'):
                        ...
                        pos_info, lab = self._get_data_tuple(text, asp_term.get('term').lower(), int(asp_term.get('from')),
    ...
    def _count_pre_spaces(self, text):
        count = 0
    ...
    def _count_mid_spaces(self, text, pos):
        count = 0
    ...
    def _check_if_ranges_overlap(self, x1, x2, y1, y2):
        return x1 <= y2 and y1 <= x2

    def _get_data_tuple(self, text, asp_term, fro, to, label, word2idx):
        words = text.split()
        # Find the ids of aspect term
        ids, st, i = [], self._count_pre_spaces(text), 0
        for word in words:
            if self._check_if_ranges_overlap(st, st + len(word) - 1, fro, to - 1):
                ids.append(i)
            st = st + len(word) + self._count_mid_spaces(text, st + len(word))
            i = i + 1
        pos_info, i = [], 0
        for word in words:
            pos_info.append(self._get_abs_pos(i, ids))
            i = i + 1
        lab = None
        if label == 'negative':
            lab = 0
    ...
    def init_word_embeddings(self, word2idx):
        import numpy as np
        wt = np.random.normal(0, self.init_std, [len(word2idx), self.edim])
